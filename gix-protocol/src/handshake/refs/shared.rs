...
pub(crate) fn from_capabilities<'a>(
    ...
    for symref in symref_values {
        let (left, right) = symref.split_at(symref.find_byte(b':').ok_or_else(|| {
            message!("{symref:?} could not be parsed. A symref is expected to look like <NAME>:<target>.").raise()
        })?);
...
pub(in crate::handshake::refs) fn parse_v1(
    ...
    let (hex_hash, path) = trimmed.split_at(trimmed.find(b" ").ok_or_else(|| {
        message!("{trimmed:?} could not be parsed. A V1 ref line should be '<hex-hash> <path>'.").raise()
    })?);
    let path = &path[1..];
    if path.is_empty() {
        return Err(message!("{trimmed:?} could not be parsed. A V1 ref line should be '<hex-hash> <path>'.").raise());
    }
    match path.strip_suffix(b"^{}") {
        Some(stripped) => {
            ...
            let (previous_path, tag) = out_refs
                ...
                .ok_or_else(|| message("Expecting peeled refs to be preceded by direct refs").raise())?;
            if previous_path != stripped {
                return Err(
                    message("Expecting peeled refs to have the same base path as the previous, unpeeled one").raise(),
                );
            }
            out_refs.push(InternalRef::Peeled {
                ...
                object: gix_hash::ObjectId::from_hex(hex_hash.as_bytes())
                    .or_raise(|| message("Could not decode peeled object ID"))?,
            });
        }
        None => {
            let object = match gix_hash::ObjectId::from_hex(hex_hash.as_bytes()) {
                ...
                Err(_) if hex_hash.as_bstr() == "shallow" => {
                    let id = gix_hash::ObjectId::from_hex(path)
                        .or_raise(|| message("Could not decode shallow object ID"))?;
                    out_shallow.push(ShallowUpdate::Shallow(id));
                    return Ok(());
                }
                Err(err) => return Err(err.and_raise(message("Could not decode object ID"))),
            };
            match out_refs
                .iter()
        ...
        (Some(hex_hash), Some(path)) => {
            let id = if hex_hash == b"unborn" {
                ...
                Some(
                    gix_hash::ObjectId::from_hex(hex_hash.as_bytes())
                        .or_raise(|| message("Could not decode object ID"))?,
                )
            };
            if path.is_empty() {
                return Err(
                    message!("{trimmed:?} could not be parsed. A V2 ref line should be '<hex-hash> <path>[ (peeled|symref-target):<value>'.").raise(),
                );
            }
            ...
            for attribute in tokens.by_ref().take(2) {
                ...
                match (tokens.next(), tokens.next()) {
                    (Some(attribute), Some(value)) => {
                        if value.is_empty() {
                            return Err(message!("{trimmed:?} could not be parsed. A V2 ref line should be '<hex-hash> <path>[ (peeled|symref-target):<value>'.").raise());
                        }
                        match attribute {
                            b"peeled" => {
                                peeled = Some(
                    ...
                    _ => return Err(message!("{trimmed:?} could not be parsed. A V2 ref line should be '<hex-hash> <path>[ (peeled|symref-target):<value>'.").raise()),
                }
            }
            ...
            Ok(match (symref_target, peeled) {
                (Some(target_name), peeled) => match target_name {
                    b"(null)" => match peeled {
                        None => Ref::Direct {
                            ...
                            object: id
                                .ok_or_else(|| message("got 'unborn' while (null) was a symref target").raise())?,
                        },
                ...
                (None, Some(peeled)) => Ref::Peeled {
                    full_ref_name: path.into(),
                    object: peeled,
                    tag: id.ok_or_else(|| message("got 'unborn' as tag target").raise())?,
                },
                (None, None) => Ref::Direct {
                    object: id
                        .ok_or_else(|| message("got 'unborn' as object name of direct reference").raise())?,
                    full_ref_name: path.into(),
                },
            })
        }
        _ => Err(message!("{trimmed:?} could not be parsed. A V2 ref line should be '<hex-hash> <path>[ (peeled|symref-target):<value>'.").raise()),
    }
}
